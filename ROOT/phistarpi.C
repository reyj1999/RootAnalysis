...
//  gStyle->SetPalette(1);
  ...
  std::ofstream out("quant_results/phistarpi.txt");  ///Name of output file with info

  auto pi = TMath::Pi();
  //int bins = 12;
  int bins = 7;

  TH1F* hphistar = new TH1F("hphistar", "#varphi* IP-IP Analysis", bins, 0.9, 5.5);
  hphistar->SetTitle(""); hphistar->SetXTitle("#varphi (rad)");


  TTree *treeU = new TTree () ;
  treeU->ReadFile("PionPionTauvert.tex");
  Int_t evtNum;
  ...
  c1->SaveAs("plots/pi_pi_phistar_mc.png");
